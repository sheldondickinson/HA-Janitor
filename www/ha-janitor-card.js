class HaJanitorCard extends HTMLElement {
  ...
  _formatDuration(entity) {
    if (entity.recorder_bad_streak_days !== null && entity.recorder_bad_streak_days !== undefined) return `${entity.recorder_bad_streak_days}d rec`;
  ...
  _renderSummary() {
    ...
    const cards = [["Entities", s.entities_total], ["Stale 30d+", s.entities_stale_30_days], ["Recorder", rec.recorder_status || "–"], ["Rec annotated", rec.recorder_entities_annotated || 0], ["Broken refs", s.broken_reference_targets], ["Kept", rc.keep || 0], ["Reviewed", rc.reviewed || 0], ["Disable candidates", rc.candidate_disable || 0]];
    return `<div class="summary-grid">${cards.map(([label, value]) => `<div class="summary-card"><div class="summary-value small-value">${value ?? "–"}</div><div class="summary-label">${label}</div></div>`).join("")}</div><div class="notice">v0.4 is read-only. Recorder duration analysis is SQLite/read-only. Search waits briefly before filtering so typing keeps focus.</div>`;
  }
  ...
  _renderEntityTable() {
    ...
    return `${this._renderFilters()}<div class="table-wrap entity-table-wrap"><table class="entity-table"><thead><tr><th class="select-col">Select</th><th>Risk</th><th>Review</th><th>Entity</th><th>State</th><th>Duration</th><th>Recorder</th><th>Refs</th><th>Device</th><th>Area</th><th>Integration</th><th class="actions-col">Actions</th></tr></thead><tbody>${rows.map((entity) => {
      ...
      const recorderTip = [`Last healthy: ${entity.recorder_last_healthy_state || "–"}`, `At: ${entity.recorder_last_healthy_at || "–"}`, `Rows: ${entity.recorder_rows_examined || 0}`].join("\n");
      return `<tr><td class="select-cell"><input type="checkbox" class="rowSelect" aria-label="Select ${this._escape(entity.entity_id)}" data-entity="${this._escape(entity.entity_id)}" ${this._selected.has(entity.entity_id) ? "checked" : ""}></td><td><span class="${this._riskClass(entity.risk)}">${this._escape(entity.risk)}</span></td><td><span class="${this._dispositionClass(disposition)}">${this._escape(disposition)}</span>${entity.review_note ? `<div class="secondary note">${this._escape(entity.review_note)}</div>` : ""}</td><td class="entity-cell"><div class="primary">${this._escape(entity.entity_id)}</div><div class="secondary">${this._escape(entity.name || "")}</div></td><td><span class="${this._stateClass(entity.state)}">${this._escape(entity.state ?? "no state")}</span></td><td class="duration-cell">${this._formatDuration(entity)}</td><td class="duration-cell" title="${this._escape(recorderTip)}">${entity.recorder_bad_streak_days ?? "–"}</td><td class="refs-cell" title="${this._escape(refTitle)}">${Number(entity.reference_count || 0)}</td><td>${this._escape(entity.device_name || "–")}</td><td>${this._escape(entity.area_name || "–")}</td><td>${this._escape(entity.integration_domain || "–")}</td><td class="row-actions"><button class="mini" data-action="reviewed" data-entity="${this._escape(entity.entity_id)}">Reviewed</button><button class="mini" data-action="keep" data-entity="${this._escape(entity.entity_id)}">Keep</button><button class="mini" data-action="candidate_disable" data-entity="${this._escape(entity.entity_id)}">Disable candidate</button><button class="mini" data-action="ignore" data-entity="${this._escape(entity.entity_id)}">Ignore</button></td></tr>`;
    }).join("")}</tbody></table></div><div class="footer-note">Showing ${rows.length} entity rows. Duration uses recorder days where available; otherwise it falls back to current HA state age.</div>`;
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `<ha-card><div class="card"><div class="header"><div><h2>${this._escape(this._config.title || "HA Janitor")}</h2><div class="subtitle">Read-only v0.4.0 audit dashboard</div></div></div>${body}</div></ha-card><style>
