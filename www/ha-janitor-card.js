class HaJanitorCard extends HTMLElement {
  ...
  _filteredEntities() {
    ...
    return this._entities.filter((entity) => {
      ...
      if (search) {
        const haystack = [entity.entity_id, entity.name, entity.device_name, entity.area_name, entity.integration_domain, entity.state, entity.risk, entity.recommendation, entity.review_note, disposition]
  ...
  async _setReview(entityId, disposition) {
    ...
    if (disposition === "keep" || disposition === "do_not_touch" || disposition === "candidate_disable" || disposition === "candidate_delete_later") {
  ...
  async _bulkSetReview(disposition) {
    ...
    const ok = confirm(`Mark ${selected.length} selected entities as ${disposition}?`);
    if (!ok) return;
    for (const entityId of selected) {
      await this._hass.callWS({ type: "ha_janitor/set_entity_review", entity_id: entityId, disposition });
    }
  ...
  async _exportCsv() {
    ...
    const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = result.filename || "ha-janitor-entities.csv"; link.click(); URL.revokeObjectURL(url);
  }

  _exportJson() {
    const payload = { exported_at: new Date().toISOString(), tool: "HA Janitor", version: "0.3.0", note: "v0.3 is read-only. Export is for manual review only.", summary: this._summary, selected_entities: this._entities.filter((entity) => this._selected.has(entity.entity_id)), broken_references: this._brokenReferences };
  ...
  _renderSummary() {
    const s = this._summary || {}; const rc = s.review_counts || {};
    const cards = [["Entities", s.entities_total], ["Referenced", s.entities_referenced], ["Unreferenced", s.entities_unreferenced], ["Broken refs", s.broken_reference_targets], ["Files scanned", s.files_scanned], ["Kept", rc.keep || 0], ["Reviewed", rc.reviewed || 0], ["Ignore", rc.ignore || 0], ["Disable candidates", rc.candidate_disable || 0]];
    return `<div class="summary-grid">${cards.map(([label, value]) => `<div class="summary-card"><div class="summary-value">${value ?? "–"}</div><div class="summary-label">${label}</div></div>`).join("")}</div><div class="notice">v0.3 is read-only. Review states are persistent. No entities are disabled, hidden or deleted.</div>`;
  }
  ...
  _renderFilters() {
    ...
    return `<div class="filters">
      <input id="search" placeholder="Search entity, device, area, integration, note…" value="${this._escape(this._filters.search)}" />
  ...
  _render() {
    ...
    if (this._loading) body = `<div class="loading">Loading HA Janitor audit and review state…</div>`;
    ...
    this.shadowRoot.innerHTML = `<ha-card><div class="card"><div class="header"><div><h2>${this._escape(this._config.title || "HA Janitor")}</h2><div class="subtitle">Read-only v0.3.0 audit dashboard</div></div></div>${body}</div></ha-card><style>
...
window.customCards.push({ type: "ha-janitor-card", name: "HA Janitor", description: "Read-only audit dashboard for stale Home Assistant entities and review workflow." });
