class HaJanitorCard extends HTMLElement {
  constructor() {
    ...
    this._sort = { view: "entities", column: "duration", direction: "desc" };
  ...
  async _load() {
    ...
    try {
      ...
      this._brokenReferences = brokenReferences || [];
      this._loaded = true;
    } catch (err) {
  ...
  _setSort(column) {
    if (this._sort.view === this._view && this._sort.column === column) {
      this._sort.direction = this._sort.direction === "asc" ? "desc" : "asc";
    } else {
      this._sort = { view: this._view, column, direction: "asc" };
    }
    this._render();
  }

  _sortHeader(label, column) {
    const active = this._sort.view === this._view && this._sort.column === column;
    const arrow = active ? (this._sort.direction === "asc" ? " ▲" : " ▼") : "";
    return `<button class="sort-header" data-sort="${this._escape(column)}">${this._escape(label)}${arrow}</button>`;
  }

  _normaliseSortValue(value) {
  ...
  _entitySortValue(entity, column) {
    const disposition = entity.disposition || "unreviewed";
    const map = {
      risk: entity.risk,
      review: disposition,
      ...
      state: entity.state,
      duration: this._bestStaleDays(entity),
      recorder: entity.recorder_bad_streak_days ?? -1,
      refs: Number(entity.reference_count || 0),
      device: entity.device_name,
      area: entity.area_name,
      integration: entity.integration_domain
    };
    return this._normaliseSortValue(map[column]);
  }

  _deviceSortValue(device, column) {
    const map = {
      risk: device.risk,
      device: device.name || device.device_id,
      area: device.area_name,
      integrations: (device.integration_domains || []).join(", "),
      entities: Number(device.entity_count || 0),
      unavailable: Number(device.unavailable_entity_count || 0),
      unknown: Number(device.unknown_entity_count || 0),
      healthy: Number(device.healthy_entity_count || 0),
      recorder: Number(device.max_recorder_bad_streak_days || 0),
      refs: Number(device.reference_count || 0)
    };
    return this._normaliseSortValue(map[column]);
  }

  _integrationSortValue(entry, column) {
    const map = {
      domain: entry.domain,
      title: entry.title,
      state: entry.state,
      devices: Number(entry.device_count || 0),
      entities: Number(entry.entity_count || 0),
      unavailable: Number(entry.unavailable_count || 0),
      unknown: Number(entry.unknown_count || 0),
      recorder: Number(entry.max_recorder_bad_streak_days || 0),
      referenced: Number(entry.referenced_entity_count || 0),
      refs: Number(entry.reference_count || 0)
    };
    return this._normaliseSortValue(map[column]);
  }

  _brokenSortValue(item, column) {
    const map = { target: item.target, count: Number(item.reference_count || 0) };
    return this._normaliseSortValue(map[column]);
  }

  _sortRows(rows, getter) {
    if (this._sort.view !== this._view || !this._sort.column) return rows;
    const direction = this._sort.direction === "desc" ? -1 : 1;
    const column = this._sort.column;
    return [...rows].sort((a, b) => {
      const av = getter(a, column);
      const bv = getter(b, column);
  ...
  async _setReview(entityId, disposition) {
    ...
    if (["keep", "do_not_touch", "candidate_disable", "candidate_delete_later"].includes(disposition)) {
      const entered = prompt(`Optional note for ${entityId}`, note);
  ...
  async _bulkSetReview(disposition) {
    ...
    if (!confirm(`Mark ${selected.length} selected entities as ${disposition}?`)) return;
  ...
  async _exportCsv() {
    const result = await this._hass.callWS({ type: "ha_janitor/export_entities_csv" });
    const blob = new Blob([result.csv || ""], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = result.filename || "ha-janitor-entities.csv";
    link.click();
    URL.revokeObjectURL(url);
  }

  _exportJson() {
    const payload = { exported_at: new Date().toISOString(), tool: "HA Janitor", version: "0.4.2", note: "v0.4.2 is read-only. Capped recorder values are lower bounds caused by recorder retention.", summary: this._summary, selected_entities: this._entities.filter((entity) => this._selected.has(entity.entity_id)), broken_references: this._brokenReferences };
  ...
  _renderSummary() {
    ...
    return `<div class="summary-grid">${cards.map(([label, value]) => `<div class="summary-card"><div class="summary-value small-value">${value ?? "–"}</div><div class="summary-label">${label}</div></div>`).join("")}</div><div class="notice">v0.4.2 is read-only. Click a column header to sort ascending; click it again to sort descending. Recorder values marked with <strong>&gt;=</strong> are lower bounds.</div>`;
  }

  _renderTabs() {
    const tabs = [["entities", "Entities"], ["devices", "Devices"], ["integrations", "Integrations"], ["broken", `Broken refs (${this._brokenReferences.length})`]];
  ...
  _renderEntityTable() {
    const rows = this._sortRows(this._filteredEntities(), (row, col) => this._entitySortValue(row, col));
    return `${this._renderFilters()}<div class="table-wrap entity-table-wrap"><table class="entity-table"><thead><tr><th class="select-col">Select</th><th>${this._sortHeader("Risk", "risk")}</th><th>${this._sortHeader("Review", "review")}</th><th>${this._sortHeader("Entity", "entity")}</th><th>${this._sortHeader("State", "state")}</th><th>${this._sortHeader("Duration", "duration")}</th><th>${this._sortHeader("Recorder", "recorder")}</th><th>${this._sortHeader("Refs", "refs")}</th><th>${this._sortHeader("Device", "device")}</th><th>${this._sortHeader("Area", "area")}</th><th>${this._sortHeader("Integration", "integration")}</th><th class="actions-col">Actions</th></tr></thead><tbody>${rows.map((entity) => {
  ...
  _renderDeviceTable() {
    const rows = this._sortRows(this._devices, (row, col) => this._deviceSortValue(row, col));
    return `<div class="table-wrap"><table><thead><tr><th>${this._sortHeader("Risk", "risk")}</th><th>${this._sortHeader("Device", "device")}</th><th>${this._sortHeader("Area", "area")}</th><th>${this._sortHeader("Integrations", "integrations")}</th><th>${this._sortHeader("Entities", "entities")}</th><th>${this._sortHeader("Unavailable", "unavailable")}</th><th>${this._sortHeader("Unknown", "unknown")}</th><th>${this._sortHeader("Healthy", "healthy")}</th><th>${this._sortHeader("Max rec days", "recorder")}</th><th>${this._sortHeader("Refs", "refs")}</th><th>Recommendation</th></tr></thead><tbody>${rows.map((device) => `<tr><td><span class="${this._riskClass(device.risk)}">${this._escape(device.risk)}</span></td><td><div class="primary">${this._escape(device.name || device.device_id)}</div><div class="secondary">${this._escape([device.manufacturer, device.model].filter(Boolean).join(" / "))}</div></td><td>${this._escape(device.area_name || "–")}</td><td>${this._escape((device.integration_domains || []).join(", ") || "–")}</td><td>${device.entity_count}</td><td>${device.unavailable_entity_count}</td><td>${device.unknown_entity_count}</td><td>${device.healthy_entity_count}</td><td>${device.max_recorder_bad_streak_days || 0}</td><td>${device.reference_count || 0}</td><td><div>${this._escape(device.recommendation || "")}</div><div class="secondary">${this._escape((device.reasons || []).join("; "))}</div></td></tr>`).join("")}</tbody></table></div>`;
  }

  _renderIntegrationTable() {
    const rows = this._sortRows(this._integrations, (row, col) => this._integrationSortValue(row, col));
    return `<div class="table-wrap"><table><thead><tr><th>${this._sortHeader("Domain", "domain")}</th><th>${this._sortHeader("Title", "title")}</th><th>${this._sortHeader("State", "state")}</th><th>${this._sortHeader("Devices", "devices")}</th><th>${this._sortHeader("Entities", "entities")}</th><th>${this._sortHeader("Unavailable", "unavailable")}</th><th>${this._sortHeader("Unknown", "unknown")}</th><th>${this._sortHeader("Max rec days", "recorder")}</th><th>${this._sortHeader("Referenced entities", "referenced")}</th><th>${this._sortHeader("Refs", "refs")}</th></tr></thead><tbody>${rows.map((entry) => `<tr><td>${this._escape(entry.domain)}</td><td>${this._escape(entry.title)}</td><td><span class="state">${this._escape(entry.state)}</span></td><td>${entry.device_count}</td><td>${entry.entity_count}</td><td>${entry.unavailable_count}</td><td>${entry.unknown_count}</td><td>${entry.max_recorder_bad_streak_days || 0}</td><td>${entry.referenced_entity_count || 0}</td><td>${entry.reference_count || 0}</td></tr>`).join("")}</tbody></table></div>`;
  }

  _renderBrokenReferences() {
    const rows = this._sortRows(this._brokenReferences, (row, col) => this._brokenSortValue(row, col));
    return `<div class="table-wrap"><table><thead><tr><th>${this._sortHeader("Missing target", "target")}</th><th>${this._sortHeader("Count", "count")}</th><th>First locations</th></tr></thead><tbody>${rows.map((item) => `<tr><td><div class="primary">${this._escape(item.target)}</div></td><td>${item.reference_count}</td><td>${(item.references || []).slice(0, 5).map((ref) => `<div><strong>${this._escape(ref.file)}:${ref.line}</strong> <span class="secondary">${this._escape(ref.preview || "")}</span></div>`).join("")}</td></tr>`).join("")}</tbody></table></div><div class="footer-note">Broken references are likely stale entity IDs in YAML or dashboard storage. False positives are possible where service/action names look like entity IDs.</div>`;
  }
  ...
  _bindEvents() {
    ...
    root.querySelectorAll(".sort-header").forEach((button) => button.addEventListener("click", () => this._setSort(button.dataset.sort)));
...
window.customCards.push({ type: "ha-janitor-card", name: "HA Janitor", description: "Read-only audit dashboard for stale Home Assistant entities with sortable columns." });
